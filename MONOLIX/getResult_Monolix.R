...
predictions_list <- list()
  for (i in 1:length(indivParams$id)){
    ...
    res<-getR0(solution,indivParams[i,],typecov,timings)
    R0s_list[[i]] <-  res

    getPlotR0(res,nameproject,indivParams[i,])

    indivParams[i,c("R0","R0min","R0max")]<-res[which(res$time==(tconf-1)),c("R0","R0ICmin","R0ICmax")]
    indivParams[i,c("R0conf","R0minconf","R0maxconf")]<-res[which(res$time==tconf+21),c("R0","R0ICmin","R0ICmax")]
...
dev.off()
plot(predictions$time[which(as.numeric(predictions$time)>700)],predictions$immunised[which(as.numeric(predictions$time)>700)])
