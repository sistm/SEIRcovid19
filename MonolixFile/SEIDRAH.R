...
SpecificInit<-list(initA=1,initS=1,initI=1,initH=1)
...
#Param
Paramdist<-list(betaw1="normal",betaw2="normal",betaw3="normal",betaw4="normal",betawp1="normal",betawp2="normal",betawp3="normal",betawp4="normal")
myOde<-SetParamDistribution(myOde,Paramdist)

ParamRegressor<-list(popsize=1,timeSinceConf=1,timeSinceDeConf=1)
myOde<-SetParamIsRegressor(myOde,ParamRegressor)
# Init
#InitRegressor<-list(initH=1)
#myOde<-SetInitIsRegressor(myOde,InitRegressor)
# Data input info
path<-"./MonolixFile/SEIRAH_20200709.txt"
sep<-"\t"
header<-c("ignore","id","ignore","observation","regressor","ignore","time","ignore","regressor","regressor","ignore","obsid","ignore")

myOde<-SetDataInput(myOde,path,header,sep)

SpecificInitBloc<-c("I_0=E_0/10",
                    "H_0=E_0/10/5",
...
PopInitValue=c(b1=1.95,
               ...
               betaw1=-0.1,
               betaw2=-0.70,
               betaw3=-1.2,betaw4=-1.2,betawp1=-1,betawp2=-1,betawp3=-1,betawp4=-1.0,Dq=0.8)
...
ode_id<-GetAttackRateAtInfinity(ode_id,AttackRateInfintyFormula,popsize_name =popsize_name,NationalName = "France")
ode_id<-GetAttackRateAtSpecificDate(ode_id,SpecificDateAttackRateEquation,as.Date("2020-05-11"),popsize_name =popsize_name,NationalName = "France")
