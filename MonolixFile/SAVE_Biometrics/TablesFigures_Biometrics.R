...
### FIGURE3
  ...
  datapred<-data.frame(time=seq(as.Date("2020-03-10"), as.Date("2021-07-15"), "day"))
    ...
    datapred<-datapred[which(datapred$time<as.Date("2020-07-15"))]
    logscale=TRUE
    log_title <- ""
    if(logscale){
      log_title <- " (log-scale)"
    ...
    p1 <- ggplot(datapred, aes(x=time, y=cumulI)) +
      ...
      geom_vline(xintercept = as.Date("2020-05-11"), linetype=2, color="black") +
...
### FIGURE 5 - MAP
  time_date<-seq(as.Date("2020-03-02"), as.Date("2020-05-12"), "days")
  time<-seq(0,length(time_date)-1,1)
  x<-list()
  x[[1]] <- list(name='timeSinceConf',
                 time=time,
  ...
  ggsave(plot = pmap, file=paste("./MonolixFile/outputMonolix/",nameproject,"/graphics/map.pdf",sep=""), width=8, height=6.4, dpi = 600)
