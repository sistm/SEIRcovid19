...
nameproject<-"sierah_poisson_proj_2y_old"
newProject(modelFile = "./monolix/seirah_poisson2Y.txt",
    ...
    print(plot(temp_monolix_estim,type=1))
     ...
     dev.off()
      ...
      result30conf[i,]<-predictepidemie(binit=as.numeric(indivParams[i,2:3]), 
                                    ...
                                    newdailyMove=0,factorreductrans=1000000,obs="2Y")
...
noaction<-result_noaction
results_30conf<-result30conf
results_90conf<-result90conf
results_180conf<-result180conf
results_270conf<-result270conf
results_340conf<-result340conf
noaction<-read.table(file=paste("./monolix/outputMonolix/",nameproject,"/graphics/indicateurs_results_noaction.txt",sep=""),header=TRUE)
results_30conf<-read.table(file=paste("./monolix/outputMonolix/",nameproject,"/graphics/indicateurs_results_30conf.txt",sep=""),header=TRUE)
results_90conf<-read.table(file=paste("./monolix/outputMonolix/",nameproject,"/graphics/indicateurs_results_90conf.txt",sep=""),header=TRUE)
results_180conf<-read.table(file=paste("./monolix/outputMonolix/",nameproject,"/graphics/indicateurs_results_180conf.txt",sep=""),header=TRUE)
results_270conf<-read.table(file=paste("./monolix/outputMonolix/",nameproject,"/graphics/indicateurs_results_270conf.txt",sep=""),header=TRUE)
results_340conf<-read.table(file=paste("./monolix/outputMonolix/",nameproject,"/graphics/indicateurs_results_340conf.txt",sep=""),header=TRUE)


temp<-cbind(as.character(results_30conf$location),results_30conf$Jdepasselit,as.character(results_30conf$Ddepasselit),results_180conf$Jdepasselit,as.character(results_180conf$Ddepasselit),results_340conf$Jdepasselit,as.character(results_340conf$Ddepasselit))
library(xtable)
xtable(temp, include.rownames=FALSE)
?xtable
...
indivParams$R0<-result_noaction$R0day14
indivParams$transmission<-round(indivParams$transmission,2)
indivParams$ascertainment<-round(indivParams$ascertainment,2)
indivParams$R0<-round(as.numeric(indivParams$R0),1)
library(xtable)
xtable(indivParams)
