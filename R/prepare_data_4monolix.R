...
prepare_data_4monolix <- function(update_from_source = TRUE){
  ...
  all_nostart <- get_data_covid19_bylevel(level = "region", source3 = "SPF",
                                          update_from_source = update_from_source,
  ...
  prep1reg <- function(r){
    date_start <- all[[r]] %>% top_n(-1, date) %>% pull(date)


    temp <- all[[r]] %>%
      rename(reg_id = maille_code, I = cas_confirmes_incident, H = hospitalisation_incident) %>%
      select(reg_id, date, day, I, H)

    I0 <- all_nostart[[r]] %>% filter(date <= date_start) %>% pull(cas_confirmes_incident) %>% sum(na.rm = TRUE)
    H0 <- all_nostart[[r]] %>% filter(date <= date_start-5) %>% pull(hospitalisation_incident) %>% sum(na.rm = TRUE)
    if(is.na(I0) | I0 == 0){
      I0 <- 1
    }
    if(is.na(H0)){
    ...
    x_final <- temp %>%
      ...
      reshape2::melt(id.vars=c("reg_id", "date", "day", "init_I0", "init_H0"), variable.name="obs_id",
                     value.name = "obs") %>%
      mutate(isolation = 1*(! date < "2020-03-18"))
  }
  ...
  all_df <- all_df[!is.na(all_df$obs), ]
