...
seidrah_ode <- function(t,Y,par){
  ...
  dYdt[1] <- -b*S*(I+alpha*A)/popSize + dailyMove*S - dailyMove*S/(popSize-I-H-D) #S
  dYdt[2] <- b*S*(I+alpha*A)/popSize-E/De - dailyMove*E/(popSize-I-H-D) #E
  dYdt[3] <- r*E/De - I/Dq - I/Di -I/Mi #I
  dYdt[4] <- H/Mh + I/Mi #D
  dYdt[5] <- (I+A)/Di + H/Dh - dailyMove*R/(popSize-I-H-D) #R
  dYdt[6] <- (1-r)*E/De - A/Di - dailyMove*A/(popSize-I-H-D) #A
