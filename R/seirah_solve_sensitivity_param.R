...
par_ini <- c(b, r, alpha, De, Di, Dq, Dh, N, n,15,45,0,1.3)


interval_b= seq(0.4,1.2,0.08)
interval_r= seq(0,1,0.1)
interval_alpha= seq(1,2,1)
...
legend_par_b = "Transmission rate of ascertained cases (b)"
...
list_legend_par = list(legend_par_b ,legend_par_r,legend_par_alpha,legend_par_De,legend_par_Di,legend_par_Dq,legend_par_Dh,legend_par_N,legend_par_n)
...
list_interval_param  = list(interval_b,interval_r,interval_alpha,interval_De,interval_Di,interval_Dq,interval_Dh,interval_N,interval_n)
...
indice_param_trial = c(1,2,4,5,6)
indice_state_ini_trial = c(2,5)
...
for (ind_state_ini_to_var in indice_state_ini_trial){
  
  #Range of tested values & range of color
  val_state_ini_trial =  list_interval_state_ini[[ind_state_ini_to_var]]
  code_color = viridis(length( val_state_ini_trial))
  
  init_pert = init
  init_pert[ind_state_ini_to_var] = val_state_ini_trial[1]
  sol_pert <- seirah_solve(init_pert, t, par_ini)
  
  main_title = list_legend_IC[[ind_state_ini_to_var]]
  matplot(t,sol_pert$E,main = main_title,ylab = "E",xlab="days",type="l",col=code_color[1],ylim=c(0,max_val_E_ref))
  for (np in 1:length(val_state_ini_trial)){
    init_pert = init
    init_pert[ind_state_ini_to_var] =val_state_ini_trial[np]
    sol_pert <- seirah_solve(init_pert, t, par_ini)
    lines(t,sol_pert$E,col=code_color[np])
  }
}
...
for (ind_state_ini_to_var in indice_state_ini_trial){
  ...
  main_title = list_legend_IC[[ind_state_ini_to_var]]
  matplot(t,par_ini[2]*sol_pert$E/par_ini[4],main = main_title,ylab = "Ascertained cases (incident numbers)",xlab="days",type="l",col=code_color[1],ylim=c(0,max_val_Y1_ref))
  for (np in 1:length(val_state_ini_trial)){
    init_pert = init
    init_pert[ind_state_ini_to_var] =val_state_ini_trial[np]
    sol_pert <- seirah_solve(init_pert, t, par_ini)
    lines(t,par_ini[2]*sol_pert$E/par_ini[4],col=code_color[np])
  }
}
...
for (ind_state_ini_to_var in indice_state_ini_trial){
  ...
  main_title = list_legend_IC[[ind_state_ini_to_var]]
  matplot(t,sol_pert$I/par_ini[6],main = main_title,ylab = "Hospitalized cases (incident numbers)",xlab="days",type="l",col=code_color[1],ylim=c(0,max_val_Y2_ref))
  for (np in 1:length(val_state_ini_trial)){
    init_pert = init
    init_pert[ind_state_ini_to_var] =val_state_ini_trial[np]
    sol_pert <- seirah_solve(init_pert, t, par_ini)
    lines(t,sol_pert$I/par_ini[6],col=code_color[np])
  }
}
