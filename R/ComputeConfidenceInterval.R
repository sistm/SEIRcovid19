...
ComputeConfidenceInterval <- function(obj,indiv,pop,id,time,nb_mc,is_global=1)
{
  UseMethod("ComputeConfidenceInterval",obj)
}

ComputeConfidenceInterval.default <- function(obj,indiv,pop,id,time,nb_mc,is_global=1)
{
...
ComputeConfidenceInterval.OdeSystem <-function(systemode,indiv,pop,id,time,nb_mc,is_global=1){
  ...
  mc_res <- parallel::mclapply(X = 1:nb_mc, mc.cores=1, FUN=function(mc_cur){
    ...
    result<-SolveThroughSimulx(systemode,is_global,time,param_and_init)
    return(result)
  }
  )
