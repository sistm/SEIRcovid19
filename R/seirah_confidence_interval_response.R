...
mat_mean_q_05_95_S <- cbind(t(apply(sapply(mc_res, "[[", "S"), 1, FUN=quantile, probs = c(0.025, 0.095))),
                            matrix(rowMeans(sapply(mc_res, "[[", "S"), na.rm=TRUE), ncol=1))
mat_mean_q_05_95_E <- cbind(t(apply(sapply(mc_res, "[[", "E"), 1, FUN=quantile, probs = c(0.025, 0.095))),
                            matrix(rowMeans(sapply(mc_res, "[[", "E"), na.rm=TRUE), ncol=1))
mat_mean_q_05_95_I <- cbind(t(apply(sapply(mc_res, "[[", "I"), 1, FUN=quantile, probs = c(0.025, 0.095))),
                            matrix(rowMeans(sapply(mc_res, "[[", "I"), na.rm=TRUE), ncol=1))
mat_mean_q_05_95_R <- cbind(t(apply(sapply(mc_res, "[[", "R"), 1, FUN=quantile, probs = c(0.025, 0.095))),
                            matrix(rowMeans(sapply(mc_res, "[[", "R"), na.rm=TRUE), ncol=1))
mat_mean_q_05_95_A <- cbind(t(apply(sapply(mc_res, "[[", "A"), 1, FUN=quantile, probs = c(0.025, 0.095))),
                            matrix(rowMeans(sapply(mc_res, "[[", "A"), na.rm=TRUE), ncol=1))
mat_mean_q_05_95_H <- cbind(t(apply(sapply(mc_res, "[[", "H"), 1, FUN=quantile, probs = c(0.025, 0.095))),
                            matrix(rowMeans(sapply(mc_res, "[[", "H"), na.rm=TRUE), ncol=1))
mat_mean_q_05_95_Y1 <- cbind(t(apply(sapply(mc_res, "[[", "Y1"), 1, FUN=quantile, probs = c(0.025, 0.095))),
                            matrix(rowMeans(sapply(mc_res, "[[", "Y1"), na.rm=TRUE), ncol=1))
mat_mean_q_05_95_Y2 <- cbind(t(apply(sapply(mc_res, "[[", "Y2"), 1, FUN=quantile, probs = c(0.025, 0.095))),
                             matrix(rowMeans(sapply(mc_res, "[[", "Y2"), na.rm=TRUE), ncol=1))
