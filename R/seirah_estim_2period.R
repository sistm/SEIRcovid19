...
seirah_estim_2period <- function(binit, data=NULL,stateinit=NULL,initwithdata=TRUE,
                         ...
                         newdailyMove=0.00001,factorreductrans=3,obs="1Y",b2=1,r2=1){
  ...
  #    }else{
        H0 <- 1
    #  }
        E0 <- data[1, "initI"]*2 # Twice the number of cases
        I0 <- data[1, "initI"] # Numbers of cases
        R0 <- 0 #(0 ref)
        A0 <- data[1, "initI"]#I0 # A=I
  ...
  par <- c(transmission, ascertainment, alpha,
           De, Di, Dq, Dh, popSize, dailyMove,timeconf,lengthconf,newdailyMove,factorreductrans,b2,r2)
  res_optimal <- seirah_solve_2period(init, t, par)
...
#' @export
#' 
plot.seirah_estim_2period <- function(x,type=1){

  sol_obstime <- x$solution[which(x$solution[,"time"] %in% x$data$day), ]
  names(sol_obstime)<-c("time", "Ssim"  ,  "Esim" ,   "Isim"  ,  "Rsim"  ,  "Asim"  ,  "Hsim"  )
  ...
  if(type==2){p<-ggplot(data2plot, aes(x=time)) +
          ...
          geom_line(aes(y = Hsim, color = "SEIRAH")) +
          theme_classic() +
